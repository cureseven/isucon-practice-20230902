...
func (h *handlers) GetClasses(c echo.Context) error {
	...
	var count int
	if err := h.DB.Get(&count, "SELECT SUM(insert_count) AS COUNTS FROM `class_insert_counts` WHERE `course_id` = ?", courseID); err != nil {
		c.Logger().Error(err)
		return c.NoContent(http.StatusInternalServerError)
	}
	if count == 0 {
...
func (h *handlers) AddAnnouncement(c echo.Context) error {
	...
	if _, err := tx2.Exec("INSERT INTO `announcements` (`id`, `course_id`, `course_name`, `title`, `message`) VALUES (?, ?, ?, ?, ?)",
